...
error_code _cecb_gs_fd(cecb_path_id path, cecb_file_stat * stat)
{
	...
	stat->ml_load_address =
		(path->dir_entry.ml_load_address1 << 8) +
		path->dir_entry.ml_load_address2;
	stat->ml_exec_address =
		(path->dir_entry.ml_exec_address1 << 8) +
		path->dir_entry.ml_exec_address2;
